...
client.on("connect", async function () {
    ...
    client.subscribe(`gw-event/received_data/+/+/+/5/6`, function (err) {
        if (!err) {
        } else {
...
client.on("message", async function (topic, message, packet) {
    ...
    var pirDiffMsg = PIRDiffInfo.unpack(new t.Pointer(wirepasMessage.wirepas.packet_received_event.payload, 0))


    console.log(serialNumber, pirDiffMsg);
})